...
pub fn check_process(process: &str) -> bool {
    let check = cmd!("pgrep", process).stdout_null().run();
    check.is_ok()
}

/// restart process
pub fn restart(process: &str, args: &str) -> bool {
